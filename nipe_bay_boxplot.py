# -*- coding: utf-8 -*-
"""
Created on Wed Jul 23 10:40:14 2025

@author: jedia
"""

def create_nipe_pollution_boxplot(before_data, during_data, after_data, longitude, latitude, 
                                 ...
                                 save_path=None):
    """
    Create a box-and-whisker plot of NO2 emissions over the most polluted areas of Nipe Bay 
    ...
    try:
        ...
        # Check if 'Nipe' exists in level 2
        nipe_matches = []
        if 'NAME_2' in cuba_adm2.columns:
            nipe_matches = [name for name in cuba_adm2['NAME_2'].unique() if 'nipe' in str(name).lower()]
            if nipe_matches:
                print(f"Found Nipe-like names: {nipe_matches}")
    except Exception as e:
        print(f"Error inspecting shapefiles: {e}")

    # Define the Nipe Bay region as a bounding box from the map coordinates
    # Using the coordinates shown in the image for Nipe Bay
    nipe_bounds = [-76.20, 20.60, -75.70, 21.00]  # [min_lon, min_lat, max_lon, max_lat]
    nipe_geom = box(nipe_bounds[0], nipe_bounds[1], nipe_bounds[2], nipe_bounds[3])
    ...
    # Function to extract data points from highly polluted regions within Nipe Bay
        ...
        for i, lat in enumerate(latitude):
            for j, lon in enumerate(longitude):
                if not np.isnan(data_2d[i, j]):
                    point = Point(lon, lat)
                    # Check if point is in Nipe Bay region and exceeds pollution threshold
                    if nipe_geom.contains(point) and pollution_data[i, j] >= pollution_threshold:
    ...
    # Customize boxplot appearance with orange theme for Nipe Bay (to differentiate from other regions)
    colors = ['#a6dcef', '#a7e9af', '#ffaaa5']  # Light to darker oranges
    ...
    ax.set_title('NO₂ Columns in Most Polluted Areas of Bahía de Nipe During Cuba Blackout', fontsize=14)
    ...
    stats_data = {
        ...
        'Before Blackout': [
            format_to_3_sig_figs(nipe_stats['Before Blackout']['mean']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['median']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['q1']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['q3']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['iqr']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['p5']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['p95']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['min']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['max']),
            format_to_3_sig_figs(nipe_stats['Before Blackout']['std']),
            nipe_stats['Before Blackout']['count']
        ],
        'During Blackout': [
            format_to_3_sig_figs(nipe_stats['During Blackout']['mean']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['median']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['q1']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['q3']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['iqr']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['p5']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['p95']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['min']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['max']),
            format_to_3_sig_figs(nipe_stats['During Blackout']['std']),
            nipe_stats['During Blackout']['count']
        ],
        'After Blackout': [
            format_to_3_sig_figs(nipe_stats['After Blackout']['mean']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['median']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['q1']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['q3']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['iqr']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['p5']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['p95']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['min']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['max']),
            format_to_3_sig_figs(nipe_stats['After Blackout']['std']),
            nipe_stats['After Blackout']['count']
        ]
    }

    stats_df = pd.DataFrame(stats_data)

    print("\nNipe Bay Most Polluted Areas (Top 10%) NO₂ Statistics:")
    print("=" * 85)
    ...
    plt.tight_layout()
    plt.show()
    return fig, nipe_stats

# Function call
create_nipe_pollution_boxplot(masked_b4_blackout, masked_in_blackout, masked_aft_blackout, longitude, latitude,
                                   adm2_shapefile=r"C:\Users\jedia\TEMPO\BNDA_CUB_2010-01-01_lastupdate\BNDA_CUB_2010-01-01_lastupdate.shp", 
                                   adm1_shapefile=r"C:\Users\jedia\TEMPO\cub_adm1\CUB_adm1.shp",
                                   save_path=None)
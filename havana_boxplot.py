# -*- coding: utf-8 -*-
"""
Created on Wed Jul 23 10:35:45 2025

@author: jedia
"""

def create_havana_pollution_boxplot(before_data, during_data, after_data, longitude, latitude, 
                                   ...
                                   save_path=None):
    """
    Create a box-and-whisker plot of NO2 emissions over the most polluted areas of Havana 
    ...
    # Load Havana polygon from shapefile
    cuba_adm1 = gpd.read_file(adm1_shapefile)
    havana_geom = cuba_adm1[cuba_adm1['NAME_1'] == "La Habana"].geometry.values[0]
    
    # Load level 2 administrative boundaries to identify polluted regions
    ...
    # Function to extract data points from highly polluted regions within Havana
        ...
        for i, lat in enumerate(latitude):
            for j, lon in enumerate(longitude):
                if not np.isnan(data_2d[i, j]):
                    point = Point(lon, lat)
                    # Check if point is in Havana and exceeds pollution threshold in 'before' data
                    if havana_geom.contains(point) and pollution_data[i, j] >= pollution_threshold:
    ...
    # Customize boxplot appearance with slightly darker colors
    colors = ['#a6dcef', '#a7e9af', '#ffaaa5']  # Slightly darker pastel colors
    ...
    ax.set_title('NO₂ Columns in Most Polluted Areas of La Habana During Cuba Blackout', fontsize=14)
    ...
    stats_data = {
        ...
        'Before Blackout': [
            format_to_3_sig_figs(havana_stats['Before Blackout']['mean']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['median']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['q1']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['q3']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['iqr']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['p5']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['p95']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['min']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['max']),
            format_to_3_sig_figs(havana_stats['Before Blackout']['std']),
            havana_stats['Before Blackout']['count']
        ],
        'During Blackout': [
            format_to_3_sig_figs(havana_stats['During Blackout']['mean']),
            format_to_3_sig_figs(havana_stats['During Blackout']['median']),
            format_to_3_sig_figs(havana_stats['During Blackout']['q1']),
            format_to_3_sig_figs(havana_stats['During Blackout']['q3']),
            format_to_3_sig_figs(havana_stats['During Blackout']['iqr']),
            format_to_3_sig_figs(havana_stats['During Blackout']['p5']),
            format_to_3_sig_figs(havana_stats['During Blackout']['p95']),
            format_to_3_sig_figs(havana_stats['During Blackout']['min']),
            format_to_3_sig_figs(havana_stats['During Blackout']['max']),
            format_to_3_sig_figs(havana_stats['During Blackout']['std']),
            havana_stats['During Blackout']['count']
        ],
        'After Blackout': [
            format_to_3_sig_figs(havana_stats['After Blackout']['mean']),
            format_to_3_sig_figs(havana_stats['After Blackout']['median']),
            format_to_3_sig_figs(havana_stats['After Blackout']['q1']),
            format_to_3_sig_figs(havana_stats['After Blackout']['q3']),
            format_to_3_sig_figs(havana_stats['After Blackout']['iqr']),
            format_to_3_sig_figs(havana_stats['After Blackout']['p5']),
            format_to_3_sig_figs(havana_stats['After Blackout']['p95']),
            format_to_3_sig_figs(havana_stats['After Blackout']['min']),
            format_to_3_sig_figs(havana_stats['After Blackout']['max']),
            format_to_3_sig_figs(havana_stats['After Blackout']['std']),
            havana_stats['After Blackout']['count']
        ]
    }

    stats_df = pd.DataFrame(stats_data)

    print("\nHavana Most Polluted Areas (Top 10%) NO₂ Statistics:")
    print("=" * 85)
    ...
    plt.tight_layout()
    plt.show()
    return fig, havana_stats

# Function call
create_havana_pollution_boxplot(masked_b4_blackout, masked_in_blackout, masked_aft_blackout, longitude, latitude,
                                   adm2_shapefile=r"C:\Users\jedia\TEMPO\BNDA_CUB_2010-01-01_lastupdate\BNDA_CUB_2010-01-01_lastupdate.shp", 
                                   adm1_shapefile=r"C:\Users\jedia\TEMPO\cub_adm1\CUB_adm1.shp",
                                   save_path=None)